...
Since we are interested in how tuning a parametric model changes the paths used to match empirical results, the SPE model was chosen.
The parameter $k$ directly determines the number of paths considered, making it more interpretable than abstract metrics like search information or diffusion efficiency required for biased random walks.
...
Our simulations are much shorter in time than the empirical time series, limiting comparability tue to different timescales.
We also did not convert the simulated data to BOLD signal, which would be necessary for more accurate comparisons but requires more complicated transformations of the signal using the Balloon-Windkessel haemodynamic model \cite{balloon_windkessel}.
...
Our results regarding the correlation between k-distance matrices and empirical FC suggest that $k$ values below 10 have a stronger relationship to functional connectivity than larger $k$ values, with $k=1$ here even showing strongest correlation by far.
Simulating brain activity using the Kuramoto model achieved best results for $k=8$, also adding to this sentiment.
...
This could suggest that brain communication maximises the number of edges used for communication while still relying on near-optimal paths for transmission.
...
Other work has shown that the SPE model can be used to leverage this variability to classify individuals not only in humans, but also macaques and mice \cite{kshortest_other_species}.
...
A more thorough investigation of the path selection could be performed, e.g. by allowing free combinations between the $k$-shortest paths instead of the weighted average.
...
\noindent
Both authors conceptualized the project and drafted section \ref{sec:method}. Daniel Peña Fonseca drafted sections \ref{sec:intro} and \ref{sec:overview}.
Lukas Prader wrote the entire code used for the project and drafted sections \ref{sec:results} and \ref{sec:conclusion}.
