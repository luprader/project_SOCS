...
In particular, there is no consensus over which kinds of paths should be used for a signal to travel between ROIs.
Seeking to answer these questions, in this study we investigate information transfer using a parametric model to capture the parameter value regime for which results from simulations based on structural connectomes most accurately reflect empirical patterns of brain activity.
...
Connectome data can be used to simulate brain activity using a range of Brain Network Communication Models (BNCMs) \cite{review_methods}.\
...
This final parametric model considers that signals between two brain regions do not only travel along the most efficient route, but through the $k$ most efficient paths.
Standard shortest-path routing is recovered setting $k=1$. Unlike other models, the shortest path ensemble still preserves selectivity, focusing on the most efficient routes.
Small values of $k$, would require extensive global topological knowledge of the network, while larger values of $k$ would relax these conditions, resulting in fewer information cost.
Therefore, the parameter $k$ balances the balances the communication delay with informational demands. 
