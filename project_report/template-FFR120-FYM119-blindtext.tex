...
\noindent
\cite{empirical_network}, \cite{rich_club_2}, \cite{rich_club_original}, , \cite{white_matter}


With the goal of exploring the relation of structural connections between brain regions to functional correlations in brain activity, connectome data can be used to simulate brain activity using a range of Brain Network Communication Models (BNCMs) \cite{review_methods} \cite{small_world_brain_networks}. On the one hand, models based on routing protocol assume that the network operates with a substantial amount of information about its structure, and communications mainly occur through the most efficient (shortest) path \cite{review_brain_structure_and_function}\cite{book_graph_brain}. Alternatively, diffusion models assume that information propagates through the brain without any use of structural information, taking more inefficient paths with higher metabolic cost \cite{random_walk}. 

Parametric models are located between these extremes, using rules that balance the amount of topological information used for propagation and the metabolic cost of transmissions. Examples of this type of frameworks include linear threshold models \cite{linear_threshold}, biased random walks \cite{biased_random_walk} and shortest path ensembles \cite{kshortest_original}. A common property among them is their ability to be tuned via a parameter, measuring how far the model should lean towards diffusion-type models or routing protocol approaches.
